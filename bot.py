...
# Функция для создания кнопки с цветом
def create_button(text, callback_data=None, url=None, color=None):
    if color is None:
        color = "#0088cc"
    # Для цвета используем обычный текст, т.к. TeleBot не поддерживает цвет кнопок
    # Но мы сохраняем цвет в настройках для отображения
    if callback_data:
...
def start(message):
    ...
    # Кнопка "Купить номер"
    buy_btn = data['settings']['buttons']['buy']
    markup.add(create_button(buy_btn['text'], callback_data='buy', color=buy_btn['color']))
    
    # Кнопка "Связь с поддержкой"
    support_btn = data['settings']['buttons']['support']
    markup.add(create_button(support_btn['text'], url=SUPPORT_LINK, color=support_btn['color']))
    
    # Кнопка "Админ панель" (только для админа)
    if str(message.from_user.id) == str(ADMIN_ID):
        admin_btn = data['settings']['buttons']['admin']
        markup.add(create_button(admin_btn['text'], callback_data='admin_panel', color=admin_btn['color']))
...
def buy_menu(call):
    ...
    # Кнопка "Назад"
    back_btn = data['settings']['buttons']['back']
    markup.add(create_button(back_btn['text'], callback_data='back_to_start', color=back_btn['color']))
...
def admin_panel(call):
    ...
    markup.add(
        ...
        InlineKeyboardButton("✏️ Редактировать кнопки", callback_data='admin_edit_buttons'),
        InlineKeyboardButton("📝 Текст приветствия", callback_data='admin_edit_text'),
...
def edit_button_menu(call):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("📝 Изменить текст", callback_data=f"edit_btn_text_{key}"),
        InlineKeyboardButton("🎨 Изменить цвет", callback_data=f"edit_btn_color_{key}"),
        InlineKeyboardButton("🔙 Назад", callback_data='admin_edit_buttons')
    )
    ...
    bot.edit_message_text(
        ...
        f"📌 Название: {btn['text']}\n"
        f"🎨 Цвет: {btn['color']}\n\n"
        f"Примеры цветов: #ff0000 (красный), #00ff00 (зелёный), #0000ff (синий), #ff8800 (оранжевый)",
        call.message.chat.id, call.message.message_id, reply_markup=markup
    )

@bot.callback_query_handler(func=lambda call: call.data.startswith('edit_btn_text_'))
def edit_button_text(call):
    ...
    msg = bot.send_message(call.message.chat.id, 
        ...
        f"💡 Можно использовать эмодзи: 📱 🔥 ⭐️ ✅ ❌ 🎯 и другие\n"
        f"💡 Можно использовать премиум эмодзи: ✨ 🦄 🌟 (они работают в Telegram)"
    )
    bot.register_next_step_handler(msg, lambda m: set_button_text(m, key))

def set_button_text(msg, key):
    text = msg.text
    data['settings']['buttons'][key]['text'] = text
    save_data()
    
    bot.send_message(msg.chat.id, f"✅ Текст кнопки обновлён:\n{text}")
    # Возврат в меню редактирования кнопок
    admin_edit_buttons(msg)

@bot.callback_query_handler(func=lambda call: call.data.startswith('edit_btn_color_'))
def edit_button_color(call):
    ...
    key = call.data.split('_')[3]
    msg = bot.send_message(call.message.chat.id, 
        f"🎨 Введите цвет для кнопки:\n\n"
        f"Текущий: {data['settings']['buttons'][key]['color']}\n\n"
        f"📌 Форматы:\n"
        f"• HEX: #ff0000 (красный)\n"
        f"• HEX: #00ff00 (зелёный)\n"
        f"• HEX: #0000ff (синий)\n"
        f"• Название: красный, синий, зелёный, оранжевый\n\n"
        f"💡 Telegram не поддерживает цвет кнопок, но цвет сохраняется в настройках\n"
        f"💡 В будущем можно использовать для кастомизации интерфейса"
    )
    bot.register_next_step_handler(msg, lambda m: set_button_color(m, key))

def set_button_color(msg, key):
    color = msg.text.strip()
    
    # Конвертация названий цветов в HEX
    color_map = {
        "красный": "#ff0000",
        "зелёный": "#00ff00",
        "зеленый": "#00ff00",
    ...
    bot.send_message(msg.chat.id, f"✅ Цвет кнопки обновлён: {color}")
    admin_edit_buttons(msg)

# --- ОСТАЛЬНЫЕ ХЕНДЛЕРЫ (статистика, заказы, управление каталогом) ---
...
def admin_stats(call):
    ...
    back_btn = data['settings']['buttons']['back']
    markup.add(create_button(back_btn['text'], callback_data='admin_panel', color=back_btn['color']))
...
def order_detail(call):
    ...
    back_btn = data['settings']['buttons']['back']
    markup.add(create_button(back_btn['text'], callback_data='admin_orders', color=back_btn['color']))
...
def set_phone(msg, idx):
    ...
    try:
        markup = InlineKeyboardMarkup()
        markup.add(create_button(wait_btn['text'], callback_data=f"wait_code_{idx}", color=wait_btn['color']))
