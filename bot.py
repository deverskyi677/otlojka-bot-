...
def is_admin(user_id):
    user_id = str(user_id)
    return user_id == str(ADMIN_ID) or user_id in data.get('admins', [])
...
def admin_panel_after(call):
    is_main_admin = str(call.from_user.id) == str(ADMIN_ID)
    ...
    markup.add(
        ...
        InlineKeyboardButton("✏️ Все тексты", callback_data='admin_edit_all_texts', style="primary"),
    )
    
    # Только главный админ может управлять админами и юр. документами
    if is_main_admin:
        markup.add(
    ...
    markup.add(InlineKeyboardButton("💬 Рассылка", callback_data='admin_broadcast', style="primary"))
    markup.add(InlineKeyboardButton("🔙 Выход", callback_data='back_to_start', style="danger"))
    
    bot.edit_message_text(
        "🛠 АДМИН ПАНЕЛЬ" + (" (ГЛАВНЫЙ)" if is_main_admin else ""),
...
def admin_manage_admins(call):
    if str(call.from_user.id) != str(ADMIN_ID):
        return
    bot.answer_callback_query(call.id)
    threading.Thread(target=smooth_execute, args=(admin_manage_admins_after, call)).start()

def admin_manage_admins_after(call):
    ...
    if admins_list:
        for admin_id in admins_list:
            user_info = data['stats']['user_details'].get(admin_id, {})
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("➕ Добавить админа", callback_data='admin_add_admin', style="success"),
        InlineKeyboardButton("🗑 Удалить админа", callback_data='admin_remove_admin', style="danger"),
...
def admin_add_admin(call):
    ...
    msg = bot.send_message(call.message.chat.id, "🆔 Введите Telegram ID нового админа:")
    bot.register_next_step_handler(msg, lambda m: add_admin(m, call.message.chat.id, call.message.message_id))

def add_admin(msg, chat_id, message_id):
    try:
        new_admin = str(msg.text).strip()
        if new_admin == str(ADMIN_ID):
            bot.send_message(msg.chat.id, "❌ Это главный админ, его нельзя добавить")
            return
        if new_admin not in data.get('admins', []):
            if 'admins' not in data:
                data['admins'] = []
            data['admins'].append(new_admin)
            save_data()
            bot.edit_message_text(f"✅ Админ {new_admin} добавлен!", chat_id, message_id)
        else:
            bot.send_message(msg.chat.id, "❌ Этот админ уже есть")
        time.sleep(0.5)
...
def admin_remove_admin(call):
    ...
    for admin_id in admins_list:
        user_info = data['stats']['user_details'].get(admin_id, {})
        name = user_info.get('first_name', 'Неизвестно')
        markup.add(InlineKeyboardButton(f"❌ {admin_id} - {name}", callback_data=f"remove_admin_{admin_id}", style="danger"))
...
def admin_users_after(call):
    ...
    for uid, info in data['stats']['user_details'].items():
        if count >= 20:
            users_text += f"\n... и ещё {len(data['stats']['user_details']) - 20} пользователей"
            break
        users_text += f"🆔 {uid}\n"
...
def accept_order(call):
    ...
    bot.register_next_step_handler(msg, lambda m: set_phone(m, idx, chat_id, message_id))
...
def set_phone(msg, idx, chat_id, message_id):
    ...
    approve_text = data['settings']['texts'].get('after_approve', 
        "✅ ВАШ ПЛАТЕЖ ОДОБРЕН!\n\n📱 Ваш номер: {phone}\n\n🔐 ИНСТРУКЦИЯ:\n1️⃣ Введите номер {phone} в Telegram\n2️⃣ Вернитесь в чат с ботом\n3️⃣ Нажмите кнопку «Жду код»\n4️⃣ Я пришлю вам код подтверждения\n\n📌 ПОСЛЕ ПОЛУЧЕНИЯ КОДА:\n• Установите двухфакторную аутентификацию\n• Привяжите почту для восстановления\n• Завершите все активные сессии"
    ).format(phone=phone)
    
    try:
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(wait_btn['text'], callback_data=f"wait_code_{idx}", style="primary"))
...
def set_reject(msg, idx, chat_id, message_id):
    ...
    try:
        reject_text = data['settings']['texts'].get('reject', "❌ Ваш платеж отклонён.\nПричина: {reason}").format(reason=reason)
        bot.send_message(data['orders'][idx]['user_id'], reject_text)
    except:
        pass
...
@bot.callback_query_handler(func=lambda call: call.data == 'admin_edit_all_texts')
def admin_edit_all_texts(call):
    if not is_admin(call.from_user.id):
        return
    bot.answer_callback_query(call.id)
    threading.Thread(target=smooth_execute, args=(admin_edit_all_texts_after, call)).start()

def admin_edit_all_texts_after(call):
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        ...
        InlineKeyboardButton("📝 Текст инструкции", callback_data='edit_text_instruction', style="primary"),
        InlineKeyboardButton("📝 Текст после одобрения", callback_data='edit_text_after_approve', style="primary"),
        InlineKeyboardButton("🔙 Назад", callback_data='admin_panel', style="primary")
    )
    bot.edit_message_text(
        "🛠 РЕДАКТИРОВАНИЕ ВСЕХ ТЕКСТОВ\n\n"
...
# Обработчики для изменения текстов
@bot.callback_query_handler(func=lambda call: call.data.startswith('edit_text_'))
def edit_text_field(call):
    ...
    msg = bot.send_message(call.message.chat.id, 
        ...
        f"📌 Текущий:\n{data['settings']['texts'].get(key, 'Не задан')}"
    )
    bot.register_next_step_handler(msg, lambda m: set_text_field(m, key, call.message.chat.id, call.message.message_id))

def set_text_field(msg, key, chat_id, message_id):
    data['settings']['texts'][key] = msg.text
    save_data()
    bot.edit_message_text(f"✅ Текст обновлён!", chat_id, message_id)
    time.sleep(0.5)
    admin_edit_all_texts_after(msg)

# ============================================================
# ЮРИДИЧЕСКИЕ ДОКУМЕНТЫ (ТОЛЬКО ГЛАВНЫЙ)
# ============================================================
...
if __name__ == '__main__':
    ...
    print(f"👥 Дополнительные админы: {data.get('admins', [])}")
