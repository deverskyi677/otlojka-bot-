    ...
    if not os.path.exists('data.json'):
        with open('data.json', 'w', encoding='utf-8') as f:
            json.dump({
                "catalog": [],
                "orders": [],
                "stats": {"users": [], "visits": 0, "user_details": {}},
                "settings": {
...
def admin_users_after(call):
    ...
    for uid, info in data['stats']['user_details'].items():
        ...
        users_text += f"📅 Первый визит: {info['first_visit']}\n"
...
def add_catalog_item(msg, chat_id, message_id):
    try:
        # Поддерживаем форматы: "США - 80", "США-80", "США 80"
        text = msg.text.strip()
        # Заменяем дефисы и пробелы
        import re
...
# ============================================================
# ИЗМЕНИТЬ ТЕКСТ КНОПКИ (РАБОТАЕТ)
# ============================================================
