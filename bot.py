...
def start(message):
    ...
    markup.add(InlineKeyboardButton(buy_btn['text'], callback_data='buy'))
    
    support_btn = data['settings']['buttons']['support']
    markup.add(InlineKeyboardButton(support_btn['text'], url=SUPPORT_LINK))
    
    if str(message.from_user.id) == str(ADMIN_ID):
        admin_btn = data['settings']['buttons']['admin']
        markup.add(InlineKeyboardButton(admin_btn['text'], callback_data='admin_panel'))
...
def buy_menu(call):
    ...
    markup.add(InlineKeyboardButton(back_btn['text'], callback_data='back_to_start'))
...
def set_button_text(msg, key, chat_id, message_id):
    ...
    # Возвращаемся в меню редактирования кнопок
    markup = InlineKeyboardMarkup(row_width=1)
    for k, btn in data['settings']['buttons'].items():
        markup.add(InlineKeyboardButton(
            f"✏️ {btn['text']}", 
            callback_data=f"edit_btn_{k}"
        ))
    markup.add(InlineKeyboardButton("🔙 Назад", callback_data='admin_panel'))
...
def admin_stats(call):
    ...
    back_btn = data['settings']['buttons']['back']
    markup.add(InlineKeyboardButton(back_btn['text'], callback_data='admin_panel'))
...
def order_detail(call):
    ...
    back_btn = data['settings']['buttons']['back']
    markup.add(InlineKeyboardButton(back_btn['text'], callback_data='admin_orders'))
