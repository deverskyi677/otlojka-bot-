        ...
        with open('data.json', 'w', encoding='utf-8') as f:
            json.dump({
                ...
                "settings": {
                    ...
                    "legal": {
                        "agreement_text": "📜 Покупая номер в нашем боте, вы соглашаетесь с условиями:\n• Политика конфиденциальности\n• Публичная оферта",
...
# ============================================================
# СТАРТ - СНАЧАЛА ПРОВЕРЯЕМ СОГЛАСИЕ
# ============================================================
...
# ============================================================
# ПОКАЗАТЬ ЮРИДИЧЕСКОЕ СОГЛАШЕНИЕ
# ============================================================
...
def show_legal_agreement(message):
    ...
    if not legal.get('show_legal', True):
        # Если юридический блок отключён - сразу в меню
...
def handle_screenshot(msg, idx):
    ...
    markup.add(
        ...
        InlineKeyboardButton("✏️ Написать", callback_data=f"reply_{order_idx}")
    )
...
def admin_panel_after(call):
    ...
    markup.add(
        ...
        InlineKeyboardButton("✏️ Редакт. кнопки", callback_data='admin_edit_buttons'),
        InlineKeyboardButton("📝 Приветствие", callback_data='admin_edit_welcome'),
        InlineKeyboardButton("⚖️ Юридические", callback_data='admin_legal'),
...
def admin_legal_after(call):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("📝 Изменить текст согласия", callback_data='legal_text'),
        InlineKeyboardButton("🔗 Изменить ссылку на политику", callback_data='legal_privacy'),
        InlineKeyboardButton("🔗 Изменить ссылку на оферту", callback_data='legal_offer'),
        InlineKeyboardButton("📌 Изменить текст кнопки", callback_data='legal_button'),
        InlineKeyboardButton("🔄 Включить/отключить", callback_data='legal_toggle'),
    ...
    bot.edit_message_text(
        ...
        f"📌 Статус: {status}\n"
        f"📝 Текст согласия:\n{legal.get('agreement_text', 'Не задан')[:100]}...\n"
...
def legal_text(call):
    ...
    msg = bot.send_message(call.message.chat.id, 
        ...
        f"💡 Можно использовать HTML: <b>жирный</b>"
    )
...
def legal_privacy(call):
    ...
    msg = bot.send_message(call.message.chat.id, 
        ...
        f"💡 Пример: https://example.com/privacy"
    )
...
def legal_offer(call):
    ...
    msg = bot.send_message(call.message.chat.id, 
        ...
        f"💡 Пример: https://example.com/offer"
    )
...
# ============================================================
# ИЗМЕНИТЬ ТЕКСТ КНОПКИ СОГЛАСИЯ
# ============================================================
...
# ============================================================
# ВКЛЮЧИТЬ/ОТКЛЮЧИТЬ ЮРИДИЧЕСКИЙ БЛОК
# ============================================================
...
def order_detail_after(call):
    ...
    if order['status'] == 'waiting_approval':
        markup.add(InlineKeyboardButton("✅ Принять", callback_data=f"accept_{idx}"), InlineKeyboardButton("❌ Отклонить", callback_data=f"reject_{idx}"))
    ...
    markup.add(InlineKeyboardButton(data['settings']['buttons']['back']['text'], callback_data='admin_orders'))
    bot.edit_message_text(
...
def set_phone(msg, idx, chat_id, message_id):
    ...
    try:
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(data['settings']['buttons']['wait_code']['text'], callback_data=f"wait_code_{idx}"))
...
def delete_number_after(call):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    for idx, item in enumerate(data['catalog']):
        markup.add(InlineKeyboardButton(f"❌ {item['country']} - {item['price']}₽", callback_data=f"del_{idx}"))
    markup.add(InlineKeyboardButton(data['settings']['buttons']['back']['text'], callback_data='admin_panel'))
...
def admin_edit_buttons_after(call):
    ...
    for key, btn in data['settings']['buttons'].items():
        markup.add(InlineKeyboardButton(f"✏️ {btn['text']}", callback_data=f"edit_{key}"))
...
def edit_button_menu_after(call):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton("📝 Изменить текст", callback_data=f"edit_text_{key}"),
        InlineKeyboardButton("🎨 Изменить цвет", callback_data=f"edit_color_{key}"),
