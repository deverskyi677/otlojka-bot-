        ...
        with open('data.json', 'w', encoding='utf-8') as f:
            json.dump({
                ...
                "settings": {
                    ...
                    "buttons": {
                        "buy": {"text": "📱 Купить номер"},
                        "support": {"text": "📞 Связь с поддержкой"},
                        "admin": {"text": "🛠 Админ панель"},
                        "back": {"text": "🔙 Назад"},
                        "wait_code": {"text": "🔄 Я вернулся, жду код"}
                    }
                }
...
def start(message):
    ...
    markup.add(InlineKeyboardButton(data['settings']['buttons']['buy']['text'], callback_data='buy'))
    markup.add(InlineKeyboardButton(data['settings']['buttons']['support']['text'], url=SUPPORT_LINK))
...
def admin_edit_buttons(call):
    ...
    bot.edit_message_text(
        ...
        "Выберите кнопку для изменения текста:",
        call.message.chat.id, call.message.message_id, reply_markup=markup
    )

# ============ КНОПКА: ВЫБОР КНОПКИ ДЛЯ РЕДАКТА ============
